  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-xl font-bold text-gray-900">Mes moyens de paiement</h1>
        <button (click)="toggleForm()" class="btn-primary">+ Ajouter une carte</button>
      </div>

      @if (showForm()) {
        <div class="card p-6 max-w-md">
          <h2 class="font-semibold text-gray-900 mb-4">Nouvelle carte</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1.5">Nom sur la carte</label>
            ...
            <div class="flex gap-3">
              ...
              <button (click)="toggleForm()" class="btn-ghost">Annuler</button>
            </div>
          </div>
        </div>
      }
          ...
          @for (card of cards(); track card.id) {
            <div class="card p-4 flex items-center gap-4">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-500">Expire {{ card.expMonth }}/{{ card.expYear }}</p>
              </div>
              @if (card.isDefault) {
                <span class="badge-primary badge">Par défaut</span>
              } @else {
                <button (click)="setDefault(card.id)" class="btn-ghost text-xs">Définir par défaut</button>
              }
