  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">{{ 'admin.orders_title' | translate }}</h1>
          <p class="text-sm text-gray-500 mt-1">{{ total() }} {{ 'admin.orders_count' | translate }}</p>
        </div>
        <button (click)="exportCsv()" class="btn-ghost text-sm flex items-center gap-2 w-fit">
          ...
          {{ 'admin.orders_export_csv' | translate }}
        </button>
      </div>
      ...
      <div class="card p-4 flex flex-col sm:flex-row gap-3">
        <input [(ngModel)]="search" (ngModelChange)="load()" type="search"
          [placeholder]="'admin.orders_search_placeholder' | translate" class="input-field flex-1 text-sm" />
        <select [(ngModel)]="statusFilter" (ngModelChange)="load()" class="input-field text-sm w-auto">
          <option value="">{{ 'admin.orders_status_all' | translate }}</option>
          <option value="Pending">{{ 'orders.status_pending' | translate }}</option>
          <option value="Paid">{{ 'orders.status_paid' | translate }}</option>
          <option value="Shipped">{{ 'orders.status_shipped' | translate }}</option>
          ...
          <option value="Refunded">{{ 'orders.status_refunded' | translate }}</option>
        </select>
      ...
      <div class="card overflow-hidden">
        @if (loading()) {
          ...
          <div class="p-12 text-center text-gray-500">{{ 'admin.orders_none' | translate }}</div>
        } @else {
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="bg-gray-50 text-left">
                  ...
                  <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ 'admin.col_items' | translate }}</th>
                    ...
                    <td class="px-5 py-3.5">
                      <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                        <button (click)="downloadInvoice(order)"
                          class="p-1.5 text-gray-400 hover:text-primary hover:bg-primary-50 rounded-lg transition-colors" [title]="'admin.orders_invoice_title' | translate">
                          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                          </svg>
                        </button>
                        <button (click)="viewOrder(order)"
                          class="p-1.5 text-gray-400 hover:text-primary hover:bg-primary-50 rounded-lg transition-colors" [title]="'admin.orders_detail_title_label' | translate">
                          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          @if (totalPages() > 1) {
            <div class="px-5 py-4 border-t border-gray-100 flex items-center justify-between">
              <p class="text-sm text-gray-500">{{ 'admin.orders_page' | translate }} {{ page() }} {{ 'admin.orders_of' | translate }} {{ totalPages() }}</p>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="selectedOrder.set(null)">
          <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto" (click)="$event.stopPropagation()">
            <div class="p-6 border-b border-gray-100 flex items-center justify-between">
              <h2 class="font-bold text-gray-900">{{ 'admin.orders_detail_title' | translate }} {{ selectedOrder()!.orderNumber }}</h2>
            ...
            <div class="p-6 space-y-4">
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div><p class="text-gray-500">{{ 'admin.col_customer' | translate }}</p><p class="font-medium">{{ selectedOrder()!.customerName }}</p></div>
                <div><p class="text-gray-500">{{ 'admin.col_email' | translate }}</p><p class="font-medium">{{ selectedOrder()!.customerEmail }}</p></div>
                <div><p class="text-gray-500">{{ 'admin.col_date' | translate }}</p><p class="font-medium">{{ selectedOrder()!.createdAt | date:'d MMMM yyyy':'':'fr' }}</p></div>
                <div><p class="text-gray-500">{{ 'admin.col_status' | translate }}</p><span [class]="statusClass(selectedOrder()!.status)" class="badge text-xs">{{ statusLabel(selectedOrder()!.status) | translate }}</span></div>
              </div>
              <div class="border-t border-gray-100 pt-4">
                <p class="font-medium text-gray-900 mb-3">{{ 'admin.orders_items_label' | translate }}</p>
                <div class="space-y-2">
                ...
                <div class="border-t border-gray-100 mt-3 pt-3 flex justify-between font-bold">
                  <span>{{ 'admin.orders_total_ttc' | translate }}</span>
...
export class AdminOrdersComponent implements OnInit {
  ...
  statusLabel(s: string) {
    const map: Record<string, string> = { Pending: 'orders.status_pending', Paid: 'orders.status_paid', Shipped: 'orders.status_shipped', Completed: 'orders.status_completed', Cancelled: 'orders.status_cancelled', Refunded: 'orders.status_refunded' };
    return map[s] ?? s;
  }
